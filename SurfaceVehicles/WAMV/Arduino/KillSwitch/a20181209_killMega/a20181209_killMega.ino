...
const int ch8Pin = 20;              // PWM coming in from channel 8 of receiver
...
int tolerance = 100;                // Tolerance for PWM signal 
...
// Averaging variables (can make this look pretty later)
const int nCh8Readings = 10;
int ch8Readings[nCh8Readings];
int ch8Index = 0;
unsigned long ch8Total = 0;
int ch8Mean = 0;
...
void setup() {
  ...
  // Intitialize ch8 array of zeros
  for (int ch8Reading = 0; ch8Reading < nCh8Readings; ch8Reading++)
  {
    ch8Readings[ch8Reading] = 0;
  }
...
void loop() {
  ...
  // Take n readings
  for (int i = 0; i <= nCh8Readings; i++) {
    ch8Readings[i] = ch8PulseWidth;
    ch8Total += ch8Readings[i];
    delay(10);
  }

  // Calculate mean
  ch8Mean = ch8Total/nCh8Readings;
  ch8Total = 0;

  // Determine kill switch (ch8) state
  if(ch8Mean + tolerance > 2200 && ch8Mean - tolerance < 2200){
    // SD switch on transmitter is in the up position (unkill)
    remoteKillStatus = false;
    digitalWrite(remoteKillOutPin, LOW); 
  }
  else if (ch8Mean + tolerance > 1650 && ch8Mean - tolerance < 1650) {
    // SD switch on transmitter is in the middle position (kill)
    remoteKillStatus = true; 
    digitalWrite(remoteKillOutPin, HIGH); 
  }
  else if (ch8Mean + tolerance > 1100 && ch8Mean - tolerance < 1100) {
