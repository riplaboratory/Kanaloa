...
#define ANSWERSIZE 14  // answer size of message to send
...
int neutral = 0;              // From 0-4096, 2048 is neutral
...
String message = "";          // String variable to send motor command messages
String temp = "";             // Strng to temporarily store motor command messages
String leftServoMsg = "Q1";   // command message for left thrusters
String rightServoMsg = "Q3";  // command message for right thrusters
...
//  nh.subscribe(autoQ4sub);      // autonomous Q4 thruster output
  ...
  // I2C Communication
  Wire.begin(SLAVE_ADDR);    // join the I2C bus as a slave with address SLAVE_ADDR (master device has no paramter)
  Wire.onRequest(sendMsgs);  // when master request for data, send motor commands
