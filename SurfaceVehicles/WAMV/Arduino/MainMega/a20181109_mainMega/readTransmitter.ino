void readSwitches() {
  // Read switch channels from receiver and assign appropriate modes of operation

  // Assign global variables to local function copy
  int tolerance = 50;
  int ch5Current = ch5PulseWidth;
  int ch7Current = ch7PulseWidth;
  int ch8Current = ch8PulseWidth;

  // Determine kill switch (ch8) state
  if (ch8Current + tolerance > 2068 && ch8Current - tolerance < 2068) {
    // SD switch on transmitter is in the up position (unkill)
    remKill = 2;
    digitalWrite(remKillPin,LOW);     // set LOW to unkill (INPUT_PULLUP on other arduino)
  }
  else if (ch8Current + tolerance > 1532 && ch8Current - tolerance < 1532) {
    // SD switch on transmitter is in the middle position (kill)
    remKill = 1;
    digitalWrite(remKillPin,HIGH);    // set HIGH to kill
  }
  else if (ch8Current + tolerance > 1032 && ch8Current - tolerance < 1032) {
  ...
  if (ch7Current + tolerance > 1044 && ch7Current - tolerance < 1044) {
    ...
    revConKill = 2;
    if (remKill == 2) {
      digitalWrite(revConKillPin,LOW);      // set LOW to unkill
    }
    else {
      digitalWrite(revConKillPin,HIGH);     // kill switch is engaged anyway, might as well kill revCon as well
    }
  }
    ...
    revConKill = 1;
    digitalWrite(revConKillPin,HIGH);    // set HIGH to kill
  }
...
void readJoysticks() {
  ...
  int yawIn = map(ch1Current,1010,2050,1000,-1000);     // yaw (positive = CCW, negative = CW)
  int surgeIn = map(ch6Current,988,2020,-1000,1000);    // surge (positive = forward, negative = backward)
  int swayIn = map(ch4Current,988,2010,-1000,1000);     // sway (positive = right, negative = left)
