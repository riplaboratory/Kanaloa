/*
    ...
    Modifications:
      - Changed I2C message scheme to use C-strings instead of Strings
...
const byte ch5Pin = 50;             // PWM/PPM in from handheld RC receiver channel 5 (calibration)
const byte ch6Pin = 52;             // PWM/PPM in from handheld RC receiver channel 6 (reset)
...
//int mainBattCheck = 0;          // reflects main battery check mode (yes = 1, no = 2, invalid = 0)          ----- COMMENTED OUT AS THIS NOT BEING USED AT THE MOMENT
//int lowCurrentBattCheck = 0;    // reflects low current battery check mode (yes = 1, no = 2, invalid = 0)   ----- COMMENTED OUT AS THIS NOT BEING USED AT THE MOMENT
int calibrationCheck = 0;       // reflects calibration check mode (yes = 1, no = 2, neutral = 3, invalid = 0)
int softwareResetCheck = 0;     // reflects software reset check mode (yes = 1, no = 2, invalid = 0)
...
int ch1PulseMin = 1040;     // minimum expected pulse width of ch1 [us] (this should be individually tested for each channel)
int ch1PulseMax = 2032;     // maximum expected pulse width of ch1 [us] (this should be individually tested for each channel)
int ch2PulseMin = 1072;     // minimum expected pulse width of ch2 [us] (this should be individually tested for each channel)
int ch2PulseMax = 2080;     // maximum expected pulse width of ch2 [us] (this should be individually tested for each channel)
int ch3PulseMin = 1216;     // minimum expected pulse width of ch3 [us] (this should be individually tested for each channel)
int ch3PulseMax = 2112;     // maximum expected pulse width of ch3 [us] (this should be individually tested for each channel)
int ch5PulseMin = 1152;     // minimum expected pulse width of ch5 [us] (this should be individually tested for each channel)
int ch5PulseMax = 2144;     // maximum expected pulse width of ch5 [us] (this should be individually tested for each channel)
int ch6PulseMin = 1120;     // minimum expected pulse width of ch7 [us] (this should be individually tested for each channel)
int ch6PulseMax = 2112;     // maximum expected pulse width of ch7 [us] (this should be individually tested for each channel)
...
//char q1Msg[5] = {'Q', '1', '0', '0', '0', '0'};
//char q2Msg[5] = {'Q', '2', '0', '0', '0', '0'};
//char q3Msg[5] = {'Q', '3', '0', '0', '0', '0'};
//char q4Msg[5] = {'Q', '4', '0', '0', '0', '0'};
char motorCmds[30] = {'Q', '1', '0', '0', '0', '0', 'N',
                      'Q', '2', '0', '0', '0', '0', 'N',
                      'Q', '3', '0', '0', '0', '0', 'N',
                      'Q', '4', '0', '0', '0', '0', 'N'}; 
...
void loop() {
  ...
  // Determine operation mode (kill, calibration check, software reset check, and auto/manual) based on inputs
  determineMode();
  ...
  // Kill code goes here
    ...
    // Run calibration code if we are in calibration mode
    calibrateController(); 
  }
