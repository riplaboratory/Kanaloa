...
void readSwitches() {
  ...
  if (ch5Current + tolerance > 2200 && ch5Current - tolerance < 2200) {
    // SF switch on transmitter is in the up position (autonomous)
    mode = 2;
    digitalWrite(statePin,LOW);     // set LOW for autonomous
  }
  else if (ch5Current + tolerance > 1200 && ch5Current - tolerance < 1200) {
...
void readJoysticks() {
  ...
  int yawIn = map(ch1Current,1175,2175,1000,-1000);       // yaw (positive = CCW, negative = CW)
  int surgeIn = map(ch6Current,1230,2240,-1000,1000);     // surge (positive = forward, negative = backward)
  int swayIn = map(ch4Current,1170,2170,-1000,1000);      // sway (positive = right, negative = left)
