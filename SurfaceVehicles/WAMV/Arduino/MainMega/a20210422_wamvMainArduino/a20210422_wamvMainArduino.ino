...
#include <Watchdog.h>                   // Watchdog library to reset Arduino programatically
...
const int ch1PulseMin = 1028;     // minimum expected pulse width of ch1 [us] (this should be individually tested for each channel)
...
const int ch2PulseMax = 2080;     // maximum expected pulse width of ch2 [us] (this should be individually tested for each channel)
const int ch3PulseMin = 1216;     // minimum expected pulse width of ch3 [us] (this should be individually tested for each channel)
const int ch3PulseMax = 2112;     // maximum expected pulse width of ch3 [us] (this should be individually tested for each channel)
const int ch5PulseMin = 1152;     // minimum expected pulse width of ch5 [us] (this should be individually tested for each channel)
const int ch5PulseMax = 2144;     // maximum expected pulse width of ch5 [us] (this should be individually tested for each channel)
const int ch6PulseMin = 1120;     // minimum expected pulse width of ch7 [us] (this should be individually tested for each channel)
const int ch6PulseMax = 2112;     // maximum expected pulse width of ch7 [us] (this should be individually tested for each channel)
...
char q1Dir = 'N';                                           // variable to hold direction for Q1
char q2Dir = 'N';                                           // variable to hold direction for Q2
char cQ1Motor[6] = {'0', '0', '0', '0'};                    // C-string array to hold character form of q1Motor
char cQ2Motor[6] = {'0', '0', '0', '0'};                    // C-string array to hold character form of q2Motor
char motorMsg[15] = {'Q', '1', '0', '0', '0', '0', q1Dir,   // C-string array to hold entire motor message
                     'Q', '2', '0', '0', '0', '0', q2Dir};  // *
char temp[6] = {'0', '0', '0', '0'};                        // temporary placeholder C-string array for message C-string creation
...
// Watchdog object
Watchdog watchdog;
const unsigned long TIMEOUT_DURATION = 300000;    // amount of time [ms] to pass before Arduino resets
const unsigned long RESET_DURATION = 500;         // ???
unsigned long enabledTime;                        // variable to store time since Arduino exits setup() function
unsigned long resetTime;                          // variable to store time since Arduino last reset
...
void loop() {
  ...
  // Save current time for Watchdog 
  ...
  battery_publisher_counter ++;  // increment battery publisher count by one

  // Stop resetting Watchdog after timeout
  if (((time - enabledTime) <= TIMEOUT_DURATION) && ((time - resetTime) >= RESET_DURATION)){
    resetTime = time;
    watchdog.reset(); 
  }
