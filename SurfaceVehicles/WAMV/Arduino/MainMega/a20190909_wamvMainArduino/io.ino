...
//  q4Pub.publish(&q4Msg);
  ...
  int rightServoOut  = map(q1Out,-1000,1000,-3500,3500); //Map limits
  int leftServoOut = map(q2Out,-1000,1000,-3500,3500);
  ...
  // Send servo output to high current Arduino
  leftServoMsg += commandToMsg(leftServoOut);
  rightServoMsg += commandToMsg(rightServoOut);
  getMsgs();
  sendMsgs();
  msgReset();
...
//      pwm.setPWM(revpin,0,0);
//    }
////    else if (vel > 0 && vel < 300){
////      pwm.setPWM(fwdpin,0,300);
////      pwm.setPWM(revpin,0,0);
////    }
////
////    else if (vel < 0 && abs(vel) < 300){
////      pwm.setPWM(fwdpin,0,0);
////      pwm.setPWM(revpin,0,300);
////    }
//    
