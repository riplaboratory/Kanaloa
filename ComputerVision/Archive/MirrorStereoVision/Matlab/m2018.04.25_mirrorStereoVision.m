...
%     colorbar;
    ...
    zoom = 2;               % zoom coefficient (max plot forward distance in [m])
    try         
        figure(10);
        ...
        figure(10);
        hAxes = pcshow(img.ptCloud,'VerticalAxis','Y','VerticalAxisDir','Down','MarkerSize',150);
        xlabel('left-right [m]');
        ylabel('down - up [m]');
        zlabel('forward [m]');
        xlim(zoom*[0,0.225]);           % left - right
        ylim(zoom*[-0.225,0.35]);       % down - up
        zlim(zoom*[0,1]);               % forward
        drawnow('limitrate');
